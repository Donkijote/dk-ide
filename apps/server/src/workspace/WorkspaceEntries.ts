...
export type WorkspaceListDirectoryError =
...
export class WorkspaceEntries extends Context.Service<
  WorkspaceEntries,
  {
    ...
    ) => Effect.Effect<ProjectListDirectoryResult, WorkspaceListDirectoryError>;
    readonly search: (
...
export const make = Effect.gen(function* () {
  ...
  const listDirectory: WorkspaceEntries["Service"]["listDirectory"] = Effect.fn(
    ...
    const dirents = yield* Effect.tryPromise({
      try: () => NodeFSP.readdir(resolved.absolutePath, { withFileTypes: true }),
      catch: (cause) =>
        new WorkspaceEntriesReadDirectoryError({
          cwd: input.cwd,
          partialPath: requestedRelativePath,
          parentPath: resolved.absolutePath,
          cause,
        }),
    });

    const parentPath = resolved.relativePath || undefined;
    const entries = dirents
      .filter((dirent) => dirent.isDirectory() || dirent.isFile())
      .map((dirent) => {
        const entryRelativePath = path
          .join(resolved.relativePath, dirent.name)
          .replaceAll("\\", "/");
        return {
  ...
  return WorkspaceEntries.of({ browse, list, listDirectory, refresh, search });
});
